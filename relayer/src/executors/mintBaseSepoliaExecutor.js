...
import { getBaseSepoliaWallet } from '../utils/provider.js';
...
/**
 * Mint Base Sepolia Executor
 * Executes mint() on WrappedZkLTC contract on Base Sepolia
 * when zkLTC is locked on LiteForge with destChainId = 84532.
 *
 * Supports batch execution with manual nonce management for concurrency.
 */
export class MintBaseSepoliaExecutor {
  constructor(txQueue) {
  ...
  _getWalletAndContract() {
    const wallet = getBaseSepoliaWallet();
  ...
  async executeBatch(txs) {
    ...
    logger.info(`[MINT-BS Worker] Processing batch of ${txs.length} transactions (baseNonce: ${baseNonce})`);
    ...
    results.forEach((result, i) => {
      if (result.status === 'rejected') {
        logger.error(`[MINT-BS Worker] Batch item failed`, { id: txs[i].id, error: result.reason?.message });
      }
    });
  }
  ...
  async _executeSingle(tx, nonce) {
    ...
    logger.info(`Executing MINT (Base Sepolia)`, {
      id,
    ...
    try {
      ...
      if (isProcessed) {
        logger.warn(`Mint already processed on Base Sepolia, marking completed`, { id });
      ...
      logger.info(`Mint tx submitted (Base Sepolia)`, { id, txHash: mintTx.hash });
      ...
      if (receipt.status === 1) {
        ...
        logger.info(`Mint CONFIRMED (Base Sepolia)`, {
          id,
          txHash: mintTx.hash,
      ...
      logger.error(`Mint FAILED (Base Sepolia)`, { id, error: errorMsg });
