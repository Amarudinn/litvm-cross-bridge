 ...
 * @dev Multi-chain version: Locks native zkLTC for cross-chain bridging.
 *      Supports multiple destination chains (Sepolia, Base Sepolia, etc.)
 *      Relayer watches Locked events and mints wzkLTC on the specified destination chain.
 *      Relayer calls unlock() when wzkLTC is burned on any supported chain.
 */
contract BridgeVaultV2 is Ownable, ReentrancyGuard, Pausable {
    // ============================================================
    //  State Variables
    // ============================================================
    ...
    /// @notice List of supported chain IDs (for enumeration)
    ...
    /**
     * @notice Lock native zkLTC to bridge to a destination chain
     * @param _recipient Address to receive wzkLTC on destination chain
     * @param _destChainId Destination chain ID (must be supported)
     */
    function lock(address _recipient, uint256 _destChainId) external payable nonReentrant whenNotPaused {
        if (msg.value == 0) revert ZeroAmount();
        if (msg.value < minLockAmount) revert BelowMinimum(msg.value, minLockAmount);
        if (_recipient == address(0)) revert InvalidRecipient();
        if (!supportedChains[_destChainId]) revert UnsupportedChain(_destChainId);
        ...
        emit Locked(msg.sender, _recipient, netAmount, fee, nonce, _destChainId);
    }

    /**
     * @notice Unlock zkLTC to user (called by relayer after burn on any supported chain)
     * @param _recipient Address to receive zkLTC
     * @param _amount Amount of zkLTC to unlock
     * @param _burnTxHash Transaction hash of the burn on source chain
     * @param _sourceNonce Nonce from the burn event on source chain
     */
    function unlock(
        address _recipient,
        uint256 _amount,
    ...
    function addChain(uint256 _chainId) external onlyOwner {
        if (supportedChains[_chainId]) revert ChainAlreadySupported(_chainId);
    ...
    function removeChain(uint256 _chainId) external onlyOwner {
        if (!supportedChains[_chainId]) revert ChainNotSupported(_chainId);
        supportedChains[_chainId] = false;

        // Remove from chainList array
        for (uint256 i = 0; i < chainList.length; i++) {
            if (chainList[i] == _chainId) {
